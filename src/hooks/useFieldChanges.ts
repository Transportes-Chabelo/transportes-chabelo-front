...
export function useFieldTextAreaChanges({ reference }: { reference?: React.RefObject<HTMLTextAreaElement> }) {
    const floatingLabel = React.useRef<HTMLSpanElement>(null);
    const inputRef = reference ?? React.useRef<HTMLTextAreaElement>(null);
    ...
    const onFocus: React.FocusEventHandler<HTMLTextAreaElement> = React.useCallback(() => {
        floatingLabel.current?.classList.add('floating-label-top');
    }, [floatingLabel.current]);

    const onBlur: React.FocusEventHandler<HTMLTextAreaElement> = React.useCallback(
        ({ target: { value } }) => {
            if (value.length === 0) {
                floatingLabel.current?.classList.remove('floating-label-top');
            }
